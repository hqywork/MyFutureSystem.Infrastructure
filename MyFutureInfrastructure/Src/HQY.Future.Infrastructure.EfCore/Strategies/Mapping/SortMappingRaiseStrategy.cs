...
{
    /// <summary>
    /// 实现了 <see cref="ISortOfEntityModel"/> 接口的实体模型关系映射策略。
    /// </summary>
    /// <typeparam name="TSortEntityModel">指定实体模型的类型。</typeparam>
    public class SortMappingRaiseStrategy<TSortEntityModel> : IMappingRaiseStrategy
        where TSortEntityModel : class, ISortOfEntityModel
    {
        ...
        public void Raise(ModelBuilder builder)
        {
            ...
            builder.Entity<TSortEntityModel>()
                   .Property(s => s.Sort)
                   .IsRequired();
        }
