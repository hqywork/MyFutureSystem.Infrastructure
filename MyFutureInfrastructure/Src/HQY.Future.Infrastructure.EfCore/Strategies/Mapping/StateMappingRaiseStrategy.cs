...
{
    /// <summary>
    /// 实现了 <see cref="IStateOfEntityModel"/> 接口的实体模型关系映射策略。
    /// </summary>
    /// <typeparam name="TStateEntityModel">指定实体模型的类型。</typeparam>
    public class StateMappingRaiseStrategy<TStateEntityModel> : IMappingRaiseStrategy
        where TStateEntityModel : class, IStateOfEntityModel
    {
        ...
        public virtual void Raise(ModelBuilder builder)
        {
            ...
            builder.Entity<TStateEntityModel>()
                   .Property(s=>s.State)
                   .IsRequired();
        }
